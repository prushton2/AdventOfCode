...
impl Node {
    ...
    fn crawl(&self, memo: &mut HashMap<Vec<char>, u64>, str: &[char]) -> u64 {
        match memo.get(&str.to_owned()) {
            Some(t) => return *t,
            None => {}
        }
        // println!("{:?}", memo);
        ...
        let mut node = self;
        let mut valid_orientations = 0;

        // loop over each character, climbing the tree as we go
        for i in 0..str.len() {
            node = match node.children.get(&str[i]) {
                Some(t) => t,
                None => break // here we dont find any pattern that fits the text, so return false
            };
            ...
            if node.node_type == NodeType::End {
                ...
                if remainder.len() == 0 {
                    ...
                    valid_orientations += self.crawl(memo, remainder);
                }
                
            }
        }

        memo.insert(str.to_owned(), valid_orientations);
...
fn main() {
    ...
    let mut sum = 0;
    let mut memo: HashMap<Vec<char>, u64> = [].into();
    for pattern in patterns {
        let res = towels.crawl(&mut memo, &pattern.chars().collect::<Vec<char>>());
        // println!("{res}");
        sum += res;
    }
    println!("{sum}");
