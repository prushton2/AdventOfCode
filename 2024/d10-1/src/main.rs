...
fn main() {
    ...
    for starting_location in &starting_locations {
        score_sum += explore_point(starting_location, &map, &mut vec![]);
        // break;
    }
    println!("{:?}", score_sum);
}
...
fn explore_point(
    ...
    for location in get_valid_adjacent_locations(start, &map) {
        ...
        score += explore_point(&location, map, found_trailheads);
    }
