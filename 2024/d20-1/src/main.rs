...
fn main() {
    ...
    println!("\n\nfaster paths: {}", total_faster_paths);
    println!("path length:  {}", path.len());
}

fn can_skip(distance: isize, pos: (usize, usize), direction: (isize, isize), map: &Vec<Vec<Pos>>, parents: &HashMap<(usize, usize), ((usize, usize), u64)>) -> bool {
    for dist in 1..distance { // loop up to distance making sure its all walls (d of 2: .#.; d of 3: .##.; so distance is how far away the empty tile is)
        let target = match get_target(pos, (direction.0 * dist as isize, direction.1 * dist as isize), map) {
            Some(t) => t,
            None => return false
        };
    ...
    let target = match get_target(pos, (direction.0 * distance as isize, direction.1 * distance as isize), map) {
        Some(t) => t,
        None => return false
    };
    ...
    let target_cost = parents.get(&target).unwrap().1 as i64;
    let cost = parents.get(&pos).unwrap().1 as i64;

    let savings = cost - target_cost - distance as i64;

    return target_cost + (distance as i64) < cost;
}
